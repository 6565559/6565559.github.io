...
(function(window) {
    ...
    const Renderers = {
         ...
         * @param {object} dbFields - 数据库字段（包含reportDate和createdAt）
         */
        renderHeader(info, dbFields) {
            ...
            // 使用数据库的report_date字段，如果不存在则使用info.dateRange
            const reportDate = dbFields?.reportDate
                ? new Date(dbFields.reportDate).toLocaleDateString('zh-CN', { year: 'numeric', month: 'long', day: 'numeric' })
                : info.dateRange;
            ...
            container.innerHTML = `
                ...
                <p class="date">${Utils.escapeHtml(reportDate)}</p>
                <div class="meta-info">
        ...
        renderHourlyActivity(hourlyActivity) {
            ...
            const updateColumns = () => {
                ...
                container.style.setProperty('--time-chart-columns', columns);
            };
        ...
        renderFooter(info, dbFields) {
            ...
            // 使用数据库的created_at字段，如果不存在则使用当前时间
            const generationTime = dbFields?.createdAt
                ? Utils.formatDateTime(new Date(dbFields.createdAt))
                : Utils.formatDateTime(new Date());

            // 使用数据库的report_date字段作为统计周期，如果不存在则使用info.dateRange
            const reportDate = dbFields?.reportDate
                ? new Date(dbFields.reportDate).toLocaleDateString('zh-CN', { year: 'numeric', month: 'long', day: 'numeric' })
                : info.dateRange;
            ...
            container.innerHTML = `
                <div class="footer-info">
                    ...
                    <span>📅 统计周期：${Utils.escapeHtml(reportDate)}</span>
    ...
    const ChatLogRenderer = {
        ...
        init(reportData) {
            try {
                ...
                // 执行渲染，传递数据库字段
                Renderers.renderHeader(reportInfo, _dbFields);
                ...
                Renderers.renderFooter(reportInfo, _dbFields);
